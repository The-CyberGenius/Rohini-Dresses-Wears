...
export default function BottomNav() {
  ...
  return (
    <div
      className={`md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white/90 backdrop-blur-lg border-t border-gray-200 shadow-[0_-5px_15px_rgba(0,0,0,0.05)] transition-transform duration-300 pb-safe ${isVisible ? "translate-y-0" : "translate-y-full"
        }`}
    >
        ...
        {navItems.map((item) => {
          ...
          return (
            <LinkComponent
              ...
              href={item.href}
              {...externalProps}
              className={`flex flex-col items-center justify-center w-full h-full space-y-1 relative transition-all duration-300 ${isActive ? "text-primary-600 scale-110" : "text-gray-500 hover:text-primary-500 hover:scale-105"
                }`}
            >
              <div
